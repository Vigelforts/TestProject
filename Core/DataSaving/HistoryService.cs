...
{
    internal sealed class HistoryService : MvxNotifyPropertyChanged, IHistoryService
    {
        ...
        public const string FileNameFormat = "history{0}_{1}.dat";
        ...
        {
            TopItems.Clear();
            _itemsController.ResetHistoryTopItems();

            foreach (DictionaryItem item in _items)
            {
                if (_itemsController.CanAddHistoryTopItem())
                {
                    TopItems.Add(item);
                }
                else
        ...
        private async void ClearCommandImpl()
        {
            ...
            bool clearRequestResult = await _userInteraction.ClearHistoryRequest();
